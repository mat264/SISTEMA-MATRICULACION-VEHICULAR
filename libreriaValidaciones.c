...
//FUNCIÓN MOSTRAR COMPROBAMANTES
	...
	while (!valido) {
		...
		for (int i = 0; i < 10; i++) {
			if (!isdigit(v.cedula[i])) {
				printf("Cedula invalida!!!. Debe contener solo di­gitos.\n");
				valido = 0;
				break; 
			}
		}
	...
	while (!valido) {
		...
		if (scanf("%f", &v.avaluo) == 1) {
			if (v.avaluo > 0) {
				...
				printf("Avaluo invalido!!!. Debe ser un numero positivo.\n");
			}
		} else {
	...
	while (!valido) {
		...
		if (scanf("%d", &v.multas) == 1) {
			if (v.multas >= 0) {
				...
				printf("Numero de multas invalido. Debe ser 0 o mayor.\n");
			}
		} else {
			printf("Error: Debe ingresar un numero valido.\n");
			int c;
